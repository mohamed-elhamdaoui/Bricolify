...
<div class="max-w-3xl mx-auto px-6 lg:px-8 py-12">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Add Skill</h1>
        <p class="mt-2 text-sm text-slate-500 font-light">Add a specific skill under a main category.</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/60 p-8 shadow-sm">
        <form action="{{ route('admin.skills.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" required class="w-full px-4 py-3 border border-slate-200 text-slate-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium placeholder-slate-400" placeholder="e.g. Pipe Replacement">
            </div>
            ...
            <div class="pt-4 flex items-center justify-end gap-4">
                ...
                <button type="submit" class="px-8 py-3 text-sm font-bold text-white bg-indigo-600 rounded-xl shadow hover:bg-indigo-700 transition-all">Save Skill</button>
            </div>
        </form>
    </div>
</div>
