...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="space-y-4">
        @forelse(auth()->user()->notifications()->latest()->get() as $notification)
            <div class="group relative bg-white/70 backdrop-blur-sm border {{ $notification->read_at ? 'border-white/90' : 'border-indigo-100 bg-indigo-50/30' }} rounded-[1.5rem] p-5 transition-all hover:shadow-md">
                <div class="flex gap-5 items-start">
                    {{-- Status Indicator --}}
                    @if(!$notification->read_at)
                        <div class="absolute top-6 left-2 w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]"></div>
                    @endif

                    {{-- Icon Box --}}
                    <div class="w-12 h-12 rounded-2xl {{ $notification->read_at ? 'bg-slate-50 text-slate-400' : 'bg-white text-indigo-600 border border-indigo-100' }} flex items-center justify-center shrink-0 shadow-sm">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        @if($notification->notifiable_id)
                            <div class="mt-4">
                                <a href="#" class="inline-flex items-center text-xs font-bold text-indigo-600 hover:text-indigo-700 gap-1 group/link">
                                    View Details
        ...
        @empty
            <div class="bg-white/60 backdrop-blur-xl border border-white/90 rounded-[2.5rem] shadow-sm p-16 flex flex-col items-center justify-center text-center">
                ...
                <p class="text-slate-500 font-light max-w-xs mx-auto">No new notifications to show right now. We'll alert you when something happens.</p>
            </div>
        @endforelse
    </div>
</div>
