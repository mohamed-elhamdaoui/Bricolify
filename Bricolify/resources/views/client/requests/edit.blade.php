...
<div class="max-w-3xl mx-auto px-6 lg:px-8 py-12">
    ...
    <div class="mb-8">
        <a href="{{ route('client.requests.show', $serviceRequest) }}" class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-slate-900 mb-6 transition-colors">
            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            Back to request details
        </a>
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Request</h1>
        <p class="mt-2 text-sm text-slate-500 font-light">Update your service request details.</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/60 p-8 shadow-sm relative overflow-hidden">
        ...
        <form action="{{ route('client.service-requests.update', $serviceRequest) }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $serviceRequest->title) }}" class="w-full px-4 py-3 border @error('title') border-rose-300 ring-rose-500/20 @else border-slate-200 @enderror text-slate-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium placeholder-slate-400" placeholder="e.g. Fix leaking sink pipe in Kitchen">
                    @error('title')
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id" class="w-full px-4 py-3 border @error('category_id') border-rose-300 ring-rose-500/20 @else border-slate-200 @enderror text-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all bg-white appearance-none">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $serviceRequest->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <p class="mt-2 text-sm text-rose-500 font-medium">{{ $message }}</p>
                ...
                <div>
                    <label for="image" class="block text-sm font-bold text-slate-900 mb-2">Photo (Optional)</label>
                    <p class="text-xs text-slate-500 mb-2 font-light">Upload a new photo to replace the existing one.</p>
                    @if($serviceRequest->image_url)
                        <div class="mb-3">
                            <p class="text-xs text-slate-500 mb-2">Current image:</p>
                            <img src="{{ asset('storage/' . $serviceRequest->image_url) }}" alt="{{ $serviceRequest->title }}" class="h-24 w-auto rounded-xl border border-slate-200">
                        </div>
                    @endif
                    <input type="file" id="image" name="image" accept="image/*" class="w-full px-4 py-3 border @error('image') border-rose-300 ring-rose-500/20 @else border-slate-200 @enderror text-slate-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    @error('image')
                        <p class="mt-2 text-sm text-rose-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="location" name="location" value="{{ old('location', $serviceRequest->location) }}" class="w-full pl-12 pr-4 py-3 border @error('location') border-rose-300 ring-rose-500/20 @else border-slate-200 @enderror text-slate-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-slate-400" placeholder="e.g. 123 Main St, Casablanca">
                        </div>
                        @error('location')
                            <p class="mt-2 text-sm text-rose-500 font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="datetime-local" id="scheduled_date" name="scheduled_date" value="{{ old('scheduled_date', $serviceRequest->scheduled_date ? $serviceRequest->scheduled_date->format('Y-m-d\TH:i') : '') }}" class="w-full pl-12 pr-4 py-3 border @error('scheduled_date') border-rose-300 ring-rose-500/20 @else border-slate-200 @enderror text-slate-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-slate-400">
                        </div>
            ...
            <div class="pt-6 border-t border-slate-100 flex items-center justify-end gap-4">
                <a href="{{ route('client.requests.show', $serviceRequest) }}" class="px-6 py-3 font-medium text-slate-600 hover:text-slate-900 transition-colors">Cancel</a>
